...
下面数字来自同机本地实验，测试设备为 NVIDIA GeForce RTX 4090。YOLOv8n 使用 `mean / p50 latency (ms)`；
GPT-2/Qwen 使用 `tokens/s` 口径，并在括号里保留 generation / prefill 延迟。YOLOv8n 当前已经支持
`ConvSiLU` 融合图走 CUDA；表中使用 `--optimal` 作为默认 benchmark 口径。

| 模型 / workload | 测试参数 | MiniORT CPU | MiniORT CUDA | ORT CPU | ORT CUDA |
| --- | --- | --- | --- | --- | --- |
| YOLOv8n | `--optimal` | `84.089 / 82.115 ms` | `4.219 / 4.211 ms` | `33.679 / 36.564 ms` | `1.86 / 1.86 ms` |
| GPT-2 KV cache | `--optimal` | `约 27.83 tokens/s`（generation mean `1724.52 ms`，prefill mean `136.64 ms`） | `约 227.40 tokens/s`（generation mean `422.16 ms`，prefill mean `11.13 ms`） | `约 83.46 tokens/s`（generation mean `1150.24 ms`，prefill mean `16.54 ms`） | `约 438.65 tokens/s`（generation mean `218.85 ms`，prefill mean `1.80 ms`） |
| Qwen2.5-0.5B KV cache | `--optimal` | `约 7.97 tokens/s`（generation mean `1004.04 ms`，prefill mean `110.25 ms`） | `约 61.65 tokens/s`（generation mean `129.77 ms`，prefill mean `15.16 ms`） | `约 15.45 tokens/s`（generation mean `517.85 ms`，prefill mean `52.98 ms`） | `约 171.36 tokens/s`（generation mean `46.68 ms`，prefill mean `4.75 ms`） |

优化记录：

- [CUDA 相关优化](./docs/optimization_summary.md)
